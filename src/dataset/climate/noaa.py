...
# Aggregation rules shared by both weekly and monthly rollups.
# PRCP is summed (total accumulation); temperature and wind are averaged.
_AGG_MAP = {
    "EVAP": "sum",
}
...
def aggregate_weekly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_map = {c: v for c, v in _AGG_MAP.items() if c in working_df.columns}
...
def aggregate_monthly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result = (
        working_df.set_index("date")
        .groupby("state")
        .resample("MS")  # month-start anchor, one row per calendar month
        .agg(agg_map)
        .reset_index()
    )
